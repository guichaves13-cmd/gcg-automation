"""
Karaoke-style subtitles generator.

PRIMARY METHOD: Edge-TTS WordBoundary timestamps (when audio was generated by
Edge-TTS). Gives EXACT per-word timing from the synthesizer itself — impossible
to be wrong because the TTS engine literally tells us when each word starts.

FALLBACK METHOD: Whisper with script_text forced alignment (when audio is from
elsewhere — e.g. an avatar persona MP4).

Produces an ASS file with per-word highlighting (yellow current word).
"""
...
async def generate_tts_with_word_timing(script: str, voice: str,
                                          output_audio_path: str,
                                          rate: str = "-5%") -> list:
    """Generate TTS audio AND derive exact per-word timing.

    Edge-TTS 7.x emits SentenceBoundary events (with offset+duration per
    sentence). We use those EXACT sentence timings and distribute the original
    script's words proportionally within each sentence, weighted by character
    length. Result: timestamps are anchored to the TTS engine's real output
    (not transcription guesses), and words come straight from the script
    (no hallucination possible).

    Returns: list[dict] {start, end, text} in seconds.
    """
    import edge_tts

    communicate = edge_tts.Communicate(script, voice, rate=rate)
    sentence_boundaries = []  # list of {start, end, text}
    ...
    with open(output_audio_path, "wb") as audio_file:
        async for chunk in communicate.stream():
            ...
            if ctype == "audio":
                ...
                # offset / duration in 100-nanosecond units → seconds
    ...
    return distribute_words_within_sentences(sentence_boundaries)


def distribute_words_within_sentences(sentences: list) -> list:
    """Take sentence-level boundaries and produce word-level timestamps.

    For each sentence:
      - Tokenize the sentence text into words (preserving punctuation per word).
      - Distribute words across the sentence's duration, weighted by character
        length (longer words get more time, accounting for syllable count).
    ...
    for sent in sentences:
        ...
        sentence_dur = max(0.001, end - start)
        ...
        total_chars = sum(len(t) + 1 for t in tokens)  # +1 = inter-word pause
        cumulative = 0
        for tok in tokens:
            tok_chars = len(tok) + 1
            w_start = start + (cumulative / total_chars) * sentence_dur
            w_end = start + ((cumulative + tok_chars) / total_chars) * sentence_dur
            words.append({
                "start": w_start,
                "end": w_end,
                "text": tok,
            })
            cumulative += tok_chars

    # Smooth out timing — end of word N = start of word N+1
